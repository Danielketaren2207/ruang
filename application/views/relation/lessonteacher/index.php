      ...
      <div class="col-md-12">
        <div class="card card-outline card-primary">
          <div class="card-header">
            <div class="btn-group">
              <a href="<?= site_url('relation/lessonteacher/add') ?>" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah</a>
...
$(document).ready(function() {
  ...
  table = $('#myTable').DataTable({ 
      ...
      "ajax": {
          "url": "<?= site_url('relation/lessonteacher/ajax_list')?>",
          "type": "POST"
      },
...
function delete_data(id)
{
  ...
  .then((willDelete) => {
    if (willDelete) {
      ...
      $.ajax({
        url : "<?= site_url('relation/lessonteacher/ajax_delete')?>/"+id,
        type: "POST",
        dataType: "JSON",
  ...
  if(list_id.length > 0){
    ...
    .then((willDelete) => {
      if (willDelete){
        // ajax delete data to database
        $.ajax({
          url: "<?= site_url('relation/lessonteacher/ajax_bulk_delete')?>",
          type: "POST",
          data: {id_lessonteacher:list_id},
          dataType: "JSON",
          success: function(data){
            if(data.status){
