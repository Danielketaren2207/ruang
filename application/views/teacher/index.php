      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <div class="btn-group">
              <a href="<?= site_url('teacher/add') ?>" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah</a>
              ...
              <a href="<?= site_url('teacher/export_excel') ?>" class="btn btn-success"><i class="fas fa-file-excel"></i> Export</a>
            ...
            <div class="modal-body">
                <form action="#" id="form_import">
                    <div class="alert alert-primary" role="alert">
                      ...
                      <a href="<?= site_url('teacher/download_format'); ?>"><b>disini</b></a>
                    </div>
...
$(document).ready(function() {
  ...
  table = $('#myTable').DataTable({ 
    ...
    "ajax": {
      "url": "<?= site_url('teacher/ajax_list')?>",
      "type": "POST"
    },
...
function delete_data(id)
{
  ...
  .then((willDelete) => {
    if (willDelete) {
      ...
      $.ajax({
        url : "<?= site_url('teacher/ajax_delete')?>/"+id,
        type: "POST",
        dataType: "JSON",
  ...
  if(list_id.length > 0){
    ...
    .then((willDelete) => {
      if (willDelete){
        // ajax delete data to database
        $.ajax({
          url: "<?= site_url('teacher/ajax_bulk_delete')?>",
          type: "POST",
          data: {id_teacher:list_id},
          dataType: "JSON",
          success: function(data){
            if(data.status){
...
function activated(id,name)
{
  swal({
    ...
    text: 'Data NIP akan dijadikan sebagai kata sandi akun.',
    icon: 'warning',
    buttons: true,
    dangerMode: true,
  })
  .then((willDelete) => {
    if (willDelete) {
      // ajax delete data to database
      $.ajax({
        url : "<?= site_url('teacher/activated')?>/"+id,
        type: "POST",
        dataType: "JSON",
        success: function(data){
...
function import_excel()
{
  ...
  $.ajax({
    url : "<?= site_url('teacher/import_excel')?>",
    type: "POST",
    data: formImport,
    contentType: false,
    processData: false,
